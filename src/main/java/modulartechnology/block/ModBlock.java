...
import modulartechnology.util.IRegisterable;
...
public abstract class ModBlock extends Block implements IRegisterable
{
    ...
    public void registerBlockIcons(IIconRegister register)
    {
        if (textures.length == 6)
        {
            for (int i = 0; i < 6; i++)
            {
                icons[i] = register.registerIcon(ModInfo.RESOURCE_PREFIX + this.subFolder + textures[i]);
            }
        }
        else
        {
            blockIcon = register.registerIcon(ModInfo.RESOURCE_PREFIX + this.subFolder + textures[0]);
